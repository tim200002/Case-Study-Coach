...
Start this section by asking the candidate to explaing his framework to you. Make sure that your wording fits into the previous conversation with the candidate.
`;
  }

  getCheckCompletionPrompt(): string {
    const completionPrompt = `Take the next response of the candidate to evaluate if this part of the case study is completed. To evaluate if the framework is completed you can use the following criteria:
